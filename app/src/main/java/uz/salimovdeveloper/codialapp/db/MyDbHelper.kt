...
class MyDbHelper(context: Context) : SQLiteOpenHelper(context, DB_NAME, null, DB_VERSION),
    ...
    override fun onCreate(db: SQLiteDatabase?) {
        val queryCourse =
            "create table $COURSE_TABLE ($COURSE_ID integer not null primary key autoincrement unique, $COURSE_NAME text not null, $COURSE_INFO text not null)"
        db?.execSQL(queryCourse)

        val queryMentor =
            "create table $MENTOR_TABLE ($MENTOR_ID integer not null primary key autoincrement unique, $MENTOR_NAME text not null, $MENTOR_SURNAME text not null, $MENTOR_PHONE, $MENTOR_COURSE_ID integer not null)"
        ...
        val queryStudent =
            "create table $STUDENT_TABLE ($STUDENT_ID integer not null primary key autoincrement unique, $STUDENT_NAME text not null, $STUDENT_SURNAME text not null, $STUDENT_PHONE text not null, $STUDENT_DAY text not null, $STUDENT_GROUP_ID integer not null, foreign key($STUDENT_GROUP_ID) references $GROUP_TABLE($GROUP_ID))"
    ...
    override fun addGroup(group: uz.salimovdeveloper.codialapp.models.Group) {
        val database = writableDatabase
        val contentValues = ContentValues()
    ...
    override fun getGroupById(id: Int): uz.salimovdeveloper.codialapp.models.Group {
        val database = readableDatabase
        val cursor = database.query(
            GROUP_TABLE,
            arrayOf(
    ...
    override fun deleteGroup(group: uz.salimovdeveloper.codialapp.models.Group) {
        val database = writableDatabase
    ...
    override fun editGroup(group: uz.salimovdeveloper.codialapp.models.Group) {
        val database = writableDatabase
        val contentValues = ContentValues()
    ...
    override fun editGroupOpen(group: uz.salimovdeveloper.codialapp.models.Group) {
        val database = writableDatabase
        val contentValues = ContentValues()
        contentValues.put(GROUP_ID, group.id)
        contentValues.put(GROUP_NAME, group.name)
